...
def main(args):
    ...
    # model.load('models/snake1v1/run242/trained_model/critic_10000.pth')
    episode = 0
        ...
        while True:
            ...
            if done:
                if np.sum(episode_reward[0]) > np.sum(episode_reward[1]):
                    step_reward = get_reward(pre_state, state, pre_info, info, ctrl_agent_index, reward, final_result=1)
                    p.append(1)
                elif np.sum(episode_reward[0]) < np.sum(episode_reward[1]):
                    step_reward = get_reward(pre_state, state, pre_info, info, ctrl_agent_index, reward, final_result=2)
                    p.append(0)
                else:
                    step_reward = get_reward(pre_state, state, pre_info, info, ctrl_agent_index, reward, final_result=3)
                    p.append(0)
                next_obs = np.zeros((1, 6, 6, 8))
            else:
                step_reward = get_reward(pre_state, state, pre_info, info, ctrl_agent_index, reward, final_result=0)
            ...
            # print()
            # store transitions
            trans = Transition(obs, action, step_reward, next_obs, done)
            ...
            if args.episode_length <= step or (True in done):
                ...
                env.reset()
                break
    np.save('m_p2.npy', m_p)
    np.save("m_s2.npy", m_s)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--episode_length', default=25000, type=int)
