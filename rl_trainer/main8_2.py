...
def main(args):
    ...
    while episode < args.max_episodes:  # 训练直到最大次数
        ...
        obs = get_observations(state, info, [0, 1], obs_dim, height, width)
        episode += 1
        step = 0
        ...
        while True:
            ...
            action3 = model.choose_action(obs[1])
            action = model.choose_action(obs[0])
            # print(f"action{action}")
            actions = np.append(action, action3)
            ...
            if done:
                ...
                next_obs = get_observations(next_state, info, [0, 1], obs_dim, height, width)
            ...
            # store transitions
            trans = Transition(obs[0], action, step_reward, np.array(next_obs[0]), done)
            ...
            if args.episode_length <= step or (True in done):
                ...
                plt.clf()  # 清除之前画的图
                plt.subplot(221)
                plt.plot(x, y, color='orange')  # 画出当前 ax 列表和 ay 列表中的值的图形
                plt.subplot(222)
                plt.plot(x, s, color='orange')
                plt.subplot(223)
                plt.plot(x, m_p, color='orange')
                plt.pause(0.01)  # 暂停一秒
                plt.ioff()  # 关闭画图的窗口
                env.reset()
