...
def get_observations(state, info, agents_index, obs_dim, height, width):
    ...
    for i in agents_index:
        ...
        observations[i][2:202] = head_surrounding[:]

        # beans positions
        observations[i][202:212] = beans_position[:]

        # other snake positions
        snake_heads = [snake[0] for snake in snakes_position]
        snake_heads = np.array(snake_heads[1:])
        snake_heads -= snakes_position[i][0]
        observations[i][212:214] = snake_heads.flatten()[:]

        # length
        other_index = 0 if agents_index[0] == 1 else 1
        observations[i][214] = len(info['snakes_position'][agents_index[0]])
        observations[i][215] = len(info['snakes_position'][other_index])
...
def get_reward(pre_info, info, snake_index, reward, final_result):
    ...
    for i in snake_index:
        if final_result == 1:  # done and won
            ...
            step_reward[i] -= (4000 - len(info['snakes_position'][snake_index[0]]) * 400) * t
        else:                       # not done
...
def get_surrounding(state, width, height, x, y, ctrl_agent, info):
    state = state.copy()
    state[state>=2] = 2 # 是身子
    state[info['snakes_position'][ctrl_agent][-1][0]][info['snakes_position'][ctrl_agent][-1][1]] = 0
    o_y = info['snakes_position'][1-ctrl_agent][0][0]
    o_x = info['snakes_position'][1-ctrl_agent][0][1]
    if state[(o_y+1)%height][o_x]!=1 and state[(y-1)%height][x]!=1 and state[y][(x+1)%width]!=1 and state[y][(x-1)%width]!=1:
        state[info['snakes_position'][1-ctrl_agent][-1][0]][info['snakes_position'][1-ctrl_agent][-1][1]] = 0
    ...
    surrounding = np.zeros((40,5))

    surrounding[0][state[(y - 2) % height][x]] = 3  # upup
    surrounding[1][state[(y + 2) % height][x]] = 3
    surrounding[2][state[y][(x - 2) % width]] = 3
    surrounding[3][state[y][(x + 2) % width]] = 3
    surrounding[4][state[(y - 1) % height][(x - 1) % width]] = 3
    surrounding[5][state[(y - 1) % height][x]] = 4
    surrounding[6][state[(y - 1) % height][(x + 1) % width]] = 3
    surrounding[7][state[y][(x - 1) % width]] = 4
    surrounding[8][state[y][(x + 1) % width]] = 4
    surrounding[9][state[(y + 1) % height][(x - 1) % width]] = 3
    surrounding[10][state[(y + 1) % height][x]] = 4
    surrounding[11][state[(y + 1) % height][(x + 1) % width]] = 3
    surrounding[12][state[(y - 3) % height][x]] = 2
    surrounding[13][state[(y + 3) % height][x]] = 2
    surrounding[14][state[y][(x - 3) % width]] = 2
    surrounding[15][state[y][(x + 3) % width]] = 2
    surrounding[16][state[(y - 2) % height][(x - 1) % width]] = 2
    surrounding[17][state[(y - 2) % height][(x + 1) % width]] = 2
    surrounding[18][state[(y + 2) % height][(x - 1) % width]] = 2
    surrounding[19][state[(y + 2) % height][(x + 1) % width]] = 2
    surrounding[20][state[(y - 1) % height][(x - 2) % width]] = 2
    surrounding[21][state[(y - 1) % height][(x + 2) % width]] = 2
    surrounding[22][state[(y + 1) % height][(x - 2) % width]] = 2
    surrounding[23][state[(y + 1) % height][(x + 2) % width]] = 2
    surrounding[24][state[(y - 4) % height][x]] = 1
    surrounding[25][state[(y + 4) % height][x]] = 1
    surrounding[26][state[y][(x - 4) % width]] = 1
    surrounding[27][state[y][(x + 4) % width]] = 1
    surrounding[28][state[(y - 3) % height][(x - 1) % width]] = 1
    surrounding[29][state[(y - 2) % height][(x - 2) % width]] = 1
    surrounding[30][state[(y - 1) % height][(x - 3) % width]] = 1
    surrounding[31][state[(y - 3) % height][(x + 1) % width]] = 1
    surrounding[32][state[(y - 2) % height][(x + 2) % width]] = 1
    surrounding[33][state[(y - 1) % height][(x + 3) % width]] = 1
    surrounding[34][state[(y + 3) % height][(x - 1) % width]] = 1
    surrounding[35][state[(y + 2) % height][(x - 2) % width]] = 1
    surrounding[36][state[(y + 1) % height][(x - 3) % width]] = 1
    surrounding[37][state[(y + 3) % height][(x + 1) % width]] = 1
    surrounding[38][state[(y + 2) % height][(x + 2) % width]] = 1
    surrounding[39][state[(y + 1) % height][(x + 3) % width]] = 1
