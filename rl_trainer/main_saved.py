...
def main(args):
    ...
    while episode < args.max_episodes:  # 训练直到最大次数
        ...
        cur = np.array(state.copy()).squeeze()
        # print(cur)
        cur[cur>=2] = -1
        inf = np.append(obs, np.reshape(cur, -1))
        # print(inf)
        episode += 1
        step = 0
        ...
        while True:
            ...
            # store transitions
            trans = Transition(inf, actions, step_reward, np.append(next_obs, next_state), done)
            # print(inf)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_c', default=0.000001, type=float)
