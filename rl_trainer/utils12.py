...
def get_observations(state, info, agents_index, obs_dim, height, width):
    ...
    for i in agents_index:
        ...
        observations[i][2:194] = head_surrounding[:]

        # beans positions
        observations[i][194:204] = beans_position[:]

        # other snake positions
        snake_heads = [snake[0] for snake in snakes_position]
        snake_heads = np.array(snake_heads[1:])
        snake_heads -= snakes_position[i][0]
        observations[i][204:206] = snake_heads.flatten()[:]

        # length
        other_index = 0 if agents_index[0] == 1 else 1
        observations[i][206] = len(info['snakes_position'][agents_index[0]])
        observations[i][207] = len(info['snakes_position'][other_index])
...
def get_surrounding(state, width, height, x, y, ctrl_agent, info):
    ...
    surrounding = np.zeros((4, 6, 8))
    for i in range(6):
        for j in range(8):
            if state[(y + i) % 6][(x + j) % 8] < 2:
                surrounding[state[(y + i) % 6][(x + j) % 8]][i][j] = 5

    length = [0, 0]
    length[ctrl_agent] = len(info['snakes_position'][ctrl_agent])
    length[1-ctrl_agent] = len(info['snakes_position'][1-ctrl_agent])

    for a, (i, j) in enumerate(info['snakes_position'][ctrl_agent]):
        surrounding[2][(i-y)%6][(j-x)%8] = length[ctrl_agent] - a
    for a, (i, j) in enumerate(info['snakes_position'][1-ctrl_agent]):
        surrounding[3][(i-y)%6][(j-x)%8] = length[1-ctrl_agent] - a
    # state = state.copy()
    ...
    #
    #
    # surrounding = np.zeros((1, 8, 6, 8))
    # for i in range(6):
    #     for j in range(8):
    #         surrounding[state[(y+i)%6][(x+j)%8]][i][j] = 1
    #
    # # print(surrounding)
