...
def get_observations(state, info, agents_index, obs_dim, height, width):
    ...
    for i in agents_index:
        ...
        observations[i][16:18] = snake_heads.flatten()[:]

        # length
        other_index = 0 if agents_index[0] == 1 else 1
        observations[i][18] = len(info['snakes_position'][agents_index[0]])
        observations[i][19] = len(info['snakes_position'][other_index])
