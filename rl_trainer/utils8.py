...
def get_observations(state, info, agents_index, obs_dim, height, width):
    ...
    for i in agents_index:
        ...
        observations[i][2:26] = head_surrounding[:]

        # beans positions
        observations[i][26:36] = beans_position[:]

        # other snake positions
        snake_heads = [snake[0] for snake in snakes_position]
        snake_heads = np.array(snake_heads[1:])
        snake_heads -= snakes_position[i][0]
        observations[i][36:38] = snake_heads.flatten()[:]

        # length
        other_index = 0 if agents_index[0] == 1 else 1
        observations[i][38] = len(info['snakes_position'][agents_index[0]])
        observations[i][39] = len(info['snakes_position'][other_index])
...
def get_surrounding(state, width, height, x, y):
    state = state.copy()
    state[state==2] = -1 # 2是自己
    state[state==3] = -1 # 3是对手
    surrounding = [state[(y - 2) % height][x],  # upup
                   state[(y + 2) % height][x],  # downdown
                   state[y][(x - 2) % width],  # leftleft
                   state[y][(x + 2) % width],
                   ...
                   state[y][(x - 3) % width],
                   state[y][(x + 3) % width],
                   state[(y - 2) % height][(x - 1) % width],
                   state[(y - 2) % height][(x + 1) % width],
                   state[(y + 2) % height][(x - 1) % width],
                   state[(y + 2) % height][(x + 1) % width],
                   state[(y - 1) % height][(x - 2) % width],
                   state[(y - 1) % height][(x + 2) % width],
                   state[(y + 1) % height][(x - 2) % width],
                   state[(y + 1) % height][(x + 2) % width]]  # rightright
