...
def get_surrounding(state, width, height, x, y, ctrl_agent, info):
    ...
    surrounding[0][state[(y - 2) % height][x]] = 2  # upup
    surrounding[1][state[(y + 2) % height][x]] = 2
    surrounding[2][state[y][(x - 2) % width]] = 2
    surrounding[3][state[y][(x + 2) % width]] = 2
    surrounding[4][state[(y - 1) % height][(x - 1) % width]] = 2
    surrounding[5][state[(y - 1) % height][x]] = 3
    surrounding[6][state[(y - 1) % height][(x + 1) % width]] = 2
    surrounding[7][state[y][(x - 1) % width]] = 3
    surrounding[8][state[y][(x + 1) % width]] = 3
    surrounding[9][state[(y + 1) % height][(x - 1) % width]] = 2
    surrounding[10][state[(y + 1) % height][x]] = 3
    surrounding[11][state[(y + 1) % height][(x + 1) % width]] = 2
